...
DLLInfo::DLLInfo(DWORD no)
{
 ...
 {
 cout << "Error: Unable to create the snapshot of current threadpool" << endl;
...
ProcessInfo::ProcessInfo()
{
 ...
 if (hProcessSnap == INVALID_HANDLE_VALUE)
 {
  ...
  cout << "Error : Unable to create the snapdhot of runningprocesses" << endl;
  return;
...
BOOL ProcessInfo::ProcessLog()
{
 const char*
	month[] = { "JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV",
	 ...
	 cout << "Time of log file creation is->" << lt.wHour << ":" << lt.wMinute << " : " << lt.wDay << "th " <<
		 month[lt.wMonth - 1] << endl;
...
BOOL ProcessInfo::ReadLog(DWORD hr, DWORD min, DWORD date, DWORD
	month)
{
 char FileName[50];
 const char*
